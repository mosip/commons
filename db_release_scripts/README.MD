## MOSIP Commons module Databases (**mosip_master, mosip_kernel, mosip_idrepo, mosip_idmap, mosip_iam, mosip_audit**) Release scripts inventory and deployment/revoke guidelines on postgresql database. 
...
* **mosip_release_commons_db_deployment.sh:**  DB release deployment script for all commons module database.

* **mosip_revoke_commons_db_deployment.sh:**  DB revoke deployment script for all commons module database deployment.

**Note:** SQL script files name under sql folder are appended with release version numbers( like **1.0.0**_master-scripts_release.sql), Same version number has to be passed as argument while executing release/revoke shell scripts. Ex. **$bash mosip_release_commons_db_deployment.sh 1.0.0**
...
**Step 2** -> DB Release deployment for all commons module databases, run the **"mosip_release_commons_db_deployment.sh"** script with release version as parameter.  To access and exeute **"mosip_release_commons_db_deployment.sh"** script, follow the below given sample commands:

<em> **Enter:-bash-4.2$** cd /home/madmin/database_release/ </em>

<em> **Enter:-bash-4.2$** bash mosip_release_commons_db_deployment.sh <release_version_number> </em>
...
<em> **Enter:-bash-4.2$** bash mosip_revoke_commons_db_deployment.sh <release_version_number> </em>
