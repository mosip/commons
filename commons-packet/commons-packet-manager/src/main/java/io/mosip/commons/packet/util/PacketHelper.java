...
public class PacketHelper {
	...
	public static boolean isSourceAndProcessPresent(String providerName, String providerSource, String providerProcess,
		...
		Optional<ProviderDto> providerDto = configurations.stream()
				.filter(dto -> dto.getSource().toLowerCase().contains(providerSource.toLowerCase())
						&& dto.getProcess().toLowerCase().contains(providerProcess.toLowerCase()))
				.findAny();
		return providerDto.isPresent() && providerDto.get() != null
				&& providerName.contains(providerDto.get().getClassName());
	}
