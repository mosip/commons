...
CMD if [ "$is_glowroot_env" = "present" ]; then \
    ...
    java -jar -javaagent:glowroot/glowroot.jar -Dloader.path=./kernel-auth-adapter.jar -Dspring.cloud.config.label="${spring_config_label_env}" -Dspring.profiles.active="${active_profile_env}" -Dspring.cloud.config.uri="${spring_config_url_env}" kernel-syncdata-service.jar ; \
    else \
    
     wget "${iam_adapter_url_env}" -O kernel-auth-adapter.jar; \
    java -jar -Dloader.path=./kernel-auth-adapter.jar -Dspring.cloud.config.label="${spring_config_label_env}" -Dspring.profiles.active="${active_profile_env}" -Dspring.cloud.config.uri="${spring_config_url_env}" kernel-syncdata-service.jar ; \
    fi
