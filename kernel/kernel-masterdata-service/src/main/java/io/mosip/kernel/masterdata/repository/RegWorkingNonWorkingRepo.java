...
public interface RegWorkingNonWorkingRepo extends BaseRepository<RegWorkingNonWorking, RegWorkingNonWorkingId>{
	...
	List<DayNameAndSeqListDto> findByregistrationCenterIdAndlanguagecodeForWeekDays(String regCenterId,String langcode);
	
	@Query("SELECT new io.mosip.kernel.masterdata.dto.getresponse.WorkingDaysDto(d.name,d.isGlobalWorking,w.dayCode,w.languagecode,w.isWorking) "+"FROM RegWorkingNonWorking w RIGHT JOIN w.daysOfWeek d "+"where w.registrationCenterId=?1 and w.languagecode=?2 and w.isActive = true")
