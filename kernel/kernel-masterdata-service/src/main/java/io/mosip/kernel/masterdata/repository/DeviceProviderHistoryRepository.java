...
public interface DeviceProviderHistoryRepository extends BaseRepository<DeviceProviderHistory, String> {
	 ...
	 */
	@Query(value = "(select * from device_provider_h dph WHERE id = ?1 AND eff_dtimes<= ?2 and isActive=true and (is_deleted is null or is_deleted =false) ORDER BY eff_dtimes DESC) LIMIT 1 ", nativeQuery = true)
