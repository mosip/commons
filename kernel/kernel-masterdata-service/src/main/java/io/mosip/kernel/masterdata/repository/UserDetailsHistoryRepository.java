...
public interface UserDetailsHistoryRepository extends BaseRepository<UserDetailsHistory, UserDetailHistoryPk> {
	...
	@Query(value = "Select 	* from master.user_detail_h m where m.regCenterId = ?1 and m.id = ?2 and m.eff_dtimes <= ?3 and ( m.is_deleted = false or m.is_deleted is null) order by m.eff_dtimes desc limit 1", nativeQuery = true)
