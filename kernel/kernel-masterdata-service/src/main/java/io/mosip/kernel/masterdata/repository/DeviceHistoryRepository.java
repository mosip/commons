...
public interface DeviceHistoryRepository extends BaseRepository<DeviceHistory, String> {
	...
	@Query(value = "Select 	d.eff_dtimes, d.id,d.cr_by, d.cr_dtimes, d.del_dtimes, d.is_active, d.is_deleted, d.upd_by, d.upd_dtimes, d.ip_address, d.lang_code, d.mac_address, d.dspec_id, d.name,  d.serial_num, d.validity_end_dtimes, d.zone_code,d.regcntr_id from master.device_master_h d where d.regcntr_id = ?1 and d.id = ?2 and d.eff_dtimes <= ?3 and ( d.is_deleted = false or d.is_deleted is null) order by d.eff_dtimes desc ", nativeQuery = true)
	List<DeviceHistory> findByFirstByRegCenterIdAndDeviceIdAndEffectDtimesLessThanEqualAndIsDeletedFalseOrIsDeletedIsNull(
