...
public interface MOSIPDeviceServiceHistoryRepository extends BaseRepository<MOSIPDeviceServiceHistory, String> {
	 ...
	 */
	@Query(value = "(select * from mosip_device_service_h dsh where sw_version = ?1 and eff_dtimes<= ?2 and (is_deleted is null or is_deleted =false) and isActive=true ORDER BY eff_dtimes DESC) LIMIT 1", nativeQuery = true)
	 ...
	 * @return {@link MOSIPDeviceServiceHistory}
	 */
	@Query(value = "(select * from mosip_device_service_h where sw_version=?1 and dtype_code=?2 and dstype_code=?3 and make=?4 and model=?5 and dprovider_id=?6 and eff_dtimes<=?7 and (is_deleted is null or is_deleted =false) and isActive=true ORDER BY eff_dtimes DESC) LIMIT 1", nativeQuery = true)
