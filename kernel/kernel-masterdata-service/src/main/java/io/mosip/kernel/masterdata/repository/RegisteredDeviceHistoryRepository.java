...
public interface RegisteredDeviceHistoryRepository extends BaseRepository<RegisteredDeviceHistory, String> {
	...
	@Query(value = "(select * from registered_device_master_h rdh WHERE code = ?1 AND eff_dtimes<= ?2 and isActive=true and(is_deleted is null or is_deleted =false) ORDER BY eff_dtimes DESC) LIMIT 1 ", nativeQuery = true)
