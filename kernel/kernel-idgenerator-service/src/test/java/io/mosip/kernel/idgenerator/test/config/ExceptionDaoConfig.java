...
@PropertySource(value = "classpath:application-exce.properties", ignoreResourceNotFound = true)
...
@ComponentScan(basePackages = { "io.mosip.kernel.vidgenerator.*","io.mosip.kernel.uingenerator.*", "io.mosip.kernel.idgenerator.vid.*",
		"io.mosip.kernel.crypto.*", "${mosip.auth.adapter.impl.basepackage}","io.mosip.kernel.idgenerator.*","io.mosip.kernel.keygenerator.bouncycastle"},excludeFilters={
				  @ComponentScan.Filter(type=FilterType.ASSIGNABLE_TYPE, value=HibernateDaoConfig.class),@ComponentScan.Filter(type=FilterType.ASSIGNABLE_TYPE, value=io.mosip.kernel.idgenerator.config.HibernateDaoConfig.class),@ComponentScan.Filter(type=FilterType.ASSIGNABLE_TYPE, value=UinNullDaoConfig.class)})
@EnableTransactionManagement
public class ExceptionDaoConfig implements EnvironmentAware {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ExceptionDaoConfig.class);
	...
	public SignatureUtil signatureUtil(){
		return Mockito.mock(SignatureUtil.class);
	}
