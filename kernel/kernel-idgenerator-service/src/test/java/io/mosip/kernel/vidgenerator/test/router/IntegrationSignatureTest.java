 ...
 * io.mosip.kernel.idgenerator.test.config.ExceptionDaoConfig; import
 ...
 * 
 * @RunWith(VertxUnitRunner.class) public class IntegrationSignatureTest {
 * 
 * private Vertx vertx; private int port; private AbstractApplicationContext
 * context; private ObjectMapper objectMapper = new ObjectMapper(); private
 * static Logger LOGGER;
 * 
 * 
 * @Before public void setup(TestContext testContext) throws IOException {
 * System.setProperty("vertx.logger-delegate-factory-class-name",
 * SLF4JLogDelegateFactory.class.getName()); objectMapper.registerModule(new
 * JavaTimeModule()); LOGGER =
 * LoggerFactory.getLogger(IntegrationSignatureTest.class); ServerSocket socket
 ...
 * AnnotationConfigApplicationContext(ExceptionDaoConfig.class);
 ...
 * vertx.deployVerticle(verticle, opts, res -> { if (res.failed()) {
 ...
 * 
 * @After public void cleanup(TestContext testContext) { if (vertx != null &&
 * testContext != null) vertx.close(testContext.asyncAssertSuccess()); if
 * (context != null) context.close(); }
 * 
 * 
 * @Test public void getUinSignatureTest(TestContext context) {
 * LOGGER.info("getUinSignatureUtilExceptionTest execution..."); Async async =
 * context.async(); AnnotationConfigApplicationContext c =
 * (AnnotationConfigApplicationContext)this.context; SignatureUtil signatureUtil
 * =c.getBean(SignatureUtil.class); SignatureResponse signatureResponse = new
 * SignatureResponse(); signatureResponse.setData("mock-signature");
 ...
 * HttpResponse<Buffer> httpResponse = ar.result(); context.assertEquals(200,
 * httpResponse.statusCode());
 * context.assertEquals(httpResponse.getHeader("response-signature"),
 * "mock-signature"); client.close(); async.complete(); } else {
