...
import io.mosip.kernel.idgenerator.test.config.UinNullDaoConfig;
...
public class UinNullExceptionTest {
	...
	public static void setup(TestContext testContext) throws IOException {
		...
		context = new AnnotationConfigApplicationContext(UinNullDaoConfig.class);
	...
	@Test
	public void getUinNullTest(TestContext context) {
		...
		client.get(port, "localhost", "/v1/idgenerator/uin").send(ar -> {
			if (ar.succeeded()) {
				...
				try {
					...
					context.assertEquals(uinResp.getErrors().get(0).getErrorCode(), UinGeneratorErrorCode.UIN_NOT_FOUND.getErrorCode());
				} catch (IOException e) {
					e.printStackTrace();
				}
				client.close();
