...
public class IdObjectSchemaValidator implements IdObjectValidator {
	 ...
	 */
	/*
	 * @Value("${mosip.kernel.idobjectvalidator.file-storage-uri}") private String
	 * configServerFileStorageURL;
	 * 
	 *//** The schema name. */
	/*
	 * @Value("${mosip.kernel.idobjectvalidator.schema-name}") private String
	 * schemaName;
	 * 
	 *//**
		 ...
		 */
	/*
	 * @Value("${mosip.kernel.idobjectvalidator.property-source}") private String
	 * propertySource;
	 * 
	 *//** The schema. */
	/*
	 ...
	 *//**
		 ...
		 */
	/*
	 * @PostConstruct public void loadSchema() throws IdObjectIOException { try { if
	 * (APPLICATION_CONTEXT.getPropertySource().equals(propertySource)) {
	 * logger.debug("schema loaded from application context"); schema =
	 * JsonLoader.fromURL(new URL(configServerFileStorageURL + schemaName)); } }
	 * catch (IOException e) { ExceptionUtils.logRootCause(e); throw new
	 * IdObjectIOException(SCHEMA_IO_EXCEPTION, e); } }
	 * 
	 *//**
		 ...
		 */
	/*
	 * @Override public boolean validateIdObject(Object idObject,
	 * IdObjectValidatorSupportedOperations operation) throws
	 * IdObjectValidationFailedException, IdObjectIOException { JsonNode
	 * jsonObjectNode = null; JsonNode jsonSchemaNode = null; ProcessingReport
	 * report = null; try { jsonObjectNode =
	 * mapper.readTree(mapper.writeValueAsString(idObject)); jsonSchemaNode =
	 * getJsonSchemaNode(); final JsonSchemaFactory factory =
	 * JsonSchemaFactory.byDefault(); final JsonSchema jsonSchema =
	 * factory.getJsonSchema(jsonSchemaNode); report =
	 * jsonSchema.validate(jsonObjectNode);
	 ...
	 * logger.debug("IdObjectValidationFailedException thrown with errors : " +
	 * errorList); throw new
	 * IdObjectValidationFailedException(ID_OBJECT_VALIDATION_FAILED, errorList); }
	 * return true; } catch (IOException e) { ExceptionUtils.logRootCause(e); throw
	 * new IdObjectIOException(ID_OBJECT_PARSING_FAILED, e); } catch
	 * (ProcessingException e) { ExceptionUtils.logRootCause(e); throw new
	 * IdObjectIOException(ID_OBJECT_VALIDATION_FAILED, e); } }
	 * 
	 *//**
		 ...
		 */
	/*
	 * private void validateMandatoryFields(JsonNode jsonObjectNode,
	 * IdObjectValidatorSupportedOperations operation, List<ServiceError> errorList)
	 * throws IdObjectIOException { if (Objects.isNull(operation)) {
	 * logger.debug("mandatory field input operation is null"); throw new
	 * IdObjectIOException(MISSING_INPUT_PARAMETER.getErrorCode(),
	 * String.format(MISSING_INPUT_PARAMETER.getMessage(), OPERATION)); } String
	 * appId = env.getProperty(APPLICATION_ID); if (Objects.isNull(appId)) {
	 * logger.debug("mandatory field input appId is null"); throw new
	 * IdObjectIOException(MISSING_INPUT_PARAMETER.getErrorCode(),
	 * String.format(MISSING_INPUT_PARAMETER.getMessage(), APPLICATION_ID)); }
	 * String fields = env.getProperty(String.format(FIELD_LIST, appId,
	 * operation.getOperation())); Optional.ofNullable(fields).ifPresent(fieldList
	 * -> Arrays.asList(StringUtils.split(fields, ','))
	 * .parallelStream().map(StringUtils::normalizeSpace).forEach(field -> {
	 * List<String> fieldNames = Arrays.asList(field.split("\\|")); fieldNames =
	 * fieldNames.stream() .map(fieldName -> PATH_SEPERATOR
	 * .concat(ROOT_PATH.concat(PATH_SEPERATOR.concat(fieldName.replace('.',
	 * '/'))))) .collect(Collectors.toList()); validateMissingFields(jsonObjectNode,
	 * errorList, fieldNames); validateInvalidFields(jsonObjectNode, errorList,
	 * fieldNames); })); }
	 * 
	 *//**
		 ...
		 */
	/*
	 * private void validateMissingFields(JsonNode jsonObjectNode,
	 * List<ServiceError> errorList, List<String> fieldNames) { if
	 * (fieldNames.parallelStream().allMatch(fieldName ->
	 * isMissingOrEmpty(jsonObjectNode, fieldName))) { errorList.add(new
	 * ServiceError(MISSING_INPUT_PARAMETER.getErrorCode(),
	 * String.format(MISSING_INPUT_PARAMETER.getMessage(),
	 * fieldNames.parallelStream().map(fieldName ->
	 * fieldName.replaceFirst(PATH_SEPERATOR, ""))
	 ...
	 *//**
		 ...
		 */
	/*
	 * private void validateInvalidFields(JsonNode jsonObjectNode,
	 * List<ServiceError> errorList, List<String> fieldNames) { if
	...
	/*
	 * private String buildErrorMessage(JsonNode processingMessageAsJson, String
	 * messageBody, String field) { return String.format(messageBody,
	 * StringUtils.strip(
	 * processingMessageAsJson.get(INSTANCE).get(POINTER).asText() +
	 * (processingMessageAsJson.hasNonNull(field) ? PATH_SEPERATOR +
	 ...
	 *//**
		 ...
		 *//*
			 ...
			 * logger.debug("propertySource is set to " + propertySource); JsonNode
			 * jsonSchemaNode = null; if
			 * (CONFIG_SERVER.getPropertySource().equals(propertySource)) { try {
			 * jsonSchemaNode = JsonLoader.fromURL(new URL(configServerFileStorageURL +
			 * schemaName)); logger.debug("schema is loaded from config server"); } catch
			 * (IOException e) { ExceptionUtils.logRootCause(e); throw new
			 * IdObjectIOException(SCHEMA_IO_EXCEPTION, e); } } else if
			 * (LOCAL.getPropertySource().equals(propertySource)) { try { jsonSchemaNode =
			 * JsonLoader.fromResource(PATH_SEPERATOR + schemaName);
			 * logger.debug("schema is loaded from LOCAL"); } catch (IOException e) {
			 * ExceptionUtils.logRootCause(e); throw new
			 * IdObjectIOException(SCHEMA_IO_EXCEPTION.getErrorCode(),
			 * SCHEMA_IO_EXCEPTION.getMessage(), e.getCause()); } } else if
			 * (APPLICATION_CONTEXT.getPropertySource().equals(propertySource)) {
			 * jsonSchemaNode = schema;
			 * logger.debug("schema is loaded from APPLICATION_CONTEXT"); } return
			 * jsonSchemaNode; }
			 */
	...
	public boolean validateIdObject(String idSchema, Object idObject)
		...
		return validateIdObject(idSchema, idObject, null);
	}
	...
	//TODO - filter out only required field errors
				...
				if (processingMessage.getLogLevel().toString().equals(ERROR)) {
					...
					if (processingMessageAsJson.hasNonNull(INSTANCE)
						...
						if (processingMessageAsJson.has(MISSING) && !processingMessageAsJson.get(MISSING).isNull()) {
							errorList.add(new ServiceError(MISSING_INPUT_PARAMETER.getErrorCode(), buildErrorMessage(
									processingMessageAsJson, MISSING_INPUT_PARAMETER.getMessage(), MISSING)));
						} else {
							errorList.add(new ServiceError(INVALID_INPUT_PARAMETER.getErrorCode(), buildErrorMessage(
	 ...
	 */	
	 private String buildErrorMessage(JsonNode processingMessageAsJson, String messageBody, String field) { 
		 return String.format(messageBody, StringUtils.strip(processingMessageAsJson.get(INSTANCE).get(POINTER).asText() +
				 (processingMessageAsJson.hasNonNull(field) ? PATH_SEPERATOR +  StringUtils.removeAll(processingMessageAsJson.get(field).toString(), 
						 "[\\[\"\\]]") : ""), "/")); 
	 }	
