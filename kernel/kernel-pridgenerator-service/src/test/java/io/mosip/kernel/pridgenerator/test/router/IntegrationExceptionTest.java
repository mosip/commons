...
import io.mosip.kernel.pridgenerator.test.config.HibernateExceptionDaoConfig;
...
public class IntegrationExceptionTest {
	...
	public static void setup(TestContext testContext) throws IOException {
		...
		context = new AnnotationConfigApplicationContext(HibernateExceptionDaoConfig.class);
		...
		AnnotationConfigApplicationContext c = (AnnotationConfigApplicationContext)context;
		PridService pridService = c.getBean(PridService.class);
	...
	@Test
	public void getPRIDExceptionTest(TestContext context) {
		LOGGER.info("getPRIDSuccessTest execution...");
		Async async = context.async();
		AnnotationConfigApplicationContext c = (AnnotationConfigApplicationContext)this.context;
		PridService pridService = c.getBean(PridService.class);
		WebClient client = WebClient.create(vertx);
		when(pridService.fetchPrid()).thenThrow(new PridGeneratorServiceException(PRIDGeneratorErrorCode.PRID_NOT_AVAILABLE.getErrorCode(),"mock-message"));
		client.get(port, "localhost", "/v1/pridgenerator/prid").send(ar -> {
			if (ar.succeeded()) {
				...
				//TODO assert error code but response is null change to empty
				client.close();
				async.complete();
			} else {
