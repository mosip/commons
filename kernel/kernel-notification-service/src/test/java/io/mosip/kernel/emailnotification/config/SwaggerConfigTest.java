...
public class SwaggerConfigTest {
	...
	private static OpenApiProperties sampleProperties() {
		...
		info.setDescription("Rest Endpoints for operations related to notification");
		info.setVersion("1.0");
		info.setLicense(license);
		Server server = new Server();
		server.setUrl("http://localhost:8083/v1/notifier");
		server.setDescription("Notification Service");
		Service service = new Service();
		service.setServers(List.of(server));
		Group group = new Group();
		group.setName("Notification Service");
		group.setPaths(List.of("/**"));
