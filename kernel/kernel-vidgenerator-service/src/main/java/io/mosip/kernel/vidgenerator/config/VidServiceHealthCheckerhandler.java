...
public class VidServiceHealthCheckerhandler implements HealthCheckHandler {
	 ...
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(VidServiceHealthCheckerhandler.class);

	public VidServiceHealthCheckerhandler(Vertx vertx, AuthProvider provider, ObjectMapper objectMapper,
		...
		this.currentWorkingDirPath = new File(System.getProperty(VIDGeneratorConstant.CURRENT_WORKING_DIRECTORY));
	...
	public void databaseHealthChecker(Future<Status> future) {
		...
		try {
			...
			final JsonObject result = resultBuilder.create().add(VIDHealthConstants.ERROR, exception.getMessage())
					.build();
				...
				try (final ResultSet rs = statement.executeQuery(DEFAULT_QUERY)) {
					...
					if (rs.next()) {
						final JsonObject result = resultBuilder.create()
								.add(VIDHealthConstants.DATABASE, conn.getMetaData().getDatabaseProductName())
								.add(VIDHealthConstants.HELLO, JdbcUtils.getResultSetValue(rs, 1)).build();
	...
	public void dispSpaceHealthChecker(Future<Status> future) {
		...
		if (diskFreeInBytes >= THRESHOLD) {
			final JsonObject result = resultBuilder.create()
					.add(VIDHealthConstants.TOTAL, this.currentWorkingDirPath.getTotalSpace())
					.add(VIDHealthConstants.FREE, diskFreeInBytes).add(VIDHealthConstants.THRESHOLD, THRESHOLD).build();
			future.complete(Status.OK(result));
		} else {
			final JsonObject result = resultBuilder.create().add(VIDHealthConstants.ERROR,
					String.format(VIDHealthConstants.THRESHOLD_ERROR, diskFreeInBytes, THRESHOLD)).build();
			future.complete(Status.KO(result));
		}
	...
	public void verticleHealthHandler(Future<Status> future, Vertx vertx) {
		...
		vertx.eventBus().send(VIDGeneratorConstant.VID_GENERATOR_ADDRESS, VIDHealthConstants.PING, response -> {
	...
	public void handle(RoutingContext rc) {
		...
		if (authProvider != null) {
			...
			if (rc.request().method() == HttpMethod.POST && rc.request().getHeader(HttpHeaders.CONTENT_TYPE) != null
					&& rc.request().getHeader(HttpHeaders.CONTENT_TYPE).contains(VIDGeneratorConstant.CONTENTTYPE)) {
	...
	private void createResponse(JsonObject json, JsonArray checks) {
		for (int i = 0; i < checks.size(); i++) {
			...
			String id = jsonobject.getString(VIDHealthConstants.ID);
			...
			try {
				if (jsonobject.containsKey(VIDHealthConstants.DATA)) {
					healthCheckModel.setDetails(jsonobject.getJsonObject(VIDHealthConstants.DATA).getMap());
					result = new JsonObject(objectMapper.writeValueAsString(healthCheckModel));

				} else {
					result = new JsonObject(objectMapper.writeValueAsString(healthCheckModel));
					result.remove(VIDHealthConstants.DETAILS);
				}
	...
	private boolean hasErrors(JsonObject json) {
		JsonObject data = json.getJsonObject(VIDHealthConstants.DATA);
	...
	private String encode(JsonObject json) {
		final String outcome = json.getString(VIDHealthConstants.OUTCOME);
		json.remove(VIDHealthConstants.OUTCOME);
