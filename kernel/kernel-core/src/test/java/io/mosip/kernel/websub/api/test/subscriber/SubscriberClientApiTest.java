...
public class SubscriberClientApiTest {
	...
	@Test
	public void subscribeLeaseSecondsZeroOkTest() throws URISyntaxException {
		subscriptionRequest.setLeaseSeconds(0);
		mockServer.expect(ExpectedCount.once(), requestTo(new URI("http://localhost:9191/hub")))
				.andExpect(method(HttpMethod.POST))
				.andRespond(withStatus(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body("hub.mode=accepted"));
		SubscriptionChangeResponse subscriptionChangeResponse = subscriptionClient.subscribe(subscriptionRequest);
	...
	@Test(expected = WebSubClientException.class)
	public void subscribeServerErrorTest() throws URISyntaxException {
		mockServer.expect(ExpectedCount.once(), requestTo(new URI("http://localhost:9191/hub")))
				.andExpect(method(HttpMethod.POST)).andRespond(withStatus(HttpStatus.INTERNAL_SERVER_ERROR).body("err"));
		subscriptionClient.subscribe(subscriptionRequest);
	}

	@Test(expected = WebSubClientException.class)
	public void unSubscribeServerErrorTest() throws URISyntaxException {
		mockServer.expect(ExpectedCount.once(), requestTo(new URI("http://localhost:9191/hub")))
	...
	@Test
	public void getFailedContentOkTest() throws Exception {
		FailedContentResponse contentResponse = new FailedContentResponse();
		FailedContentResponse.Failedcontents failedcontent = new FailedContentResponse.Failedcontents();
		failedcontent.setMessage("msg1");
		failedcontent.setTimestamp(DateUtils.getUTCCurrentDateTimeString());
		contentResponse.setFailedcontents(java.util.Collections.singletonList(failedcontent));
		mockServer.expect(ExpectedCount.once(), requestTo(new URI(expectedFailedContentURL)))
				.andExpect(method(HttpMethod.GET))
				.andRespond(withStatus(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON)
						.body(objectMapper.writeValueAsBytes(contentResponse)));
		FailedContentResponse failedContentResponse = subscriptionExtendedClient.getFailedContent(failedContentRequest);
		assertThat(failedContentResponse.getFailedcontents().size(), is(1));
	}

	@Test
	public void getFailedContentNoMessageCountAndNegativePageTest() throws Exception {
		failedContentRequest.setMessageCount(0);
		failedContentRequest.setPaginationIndex(-1);
		String url = "http://localhost:9191/hub?topic=demo&callback=aHR0cDovL2xvY2FsaG9zdDo4MDgwL2NhbGxiYWNr&timestamp="
				+ failedContentRequest.getTimestamp() + "&pageindex=0&messageCount";
		FailedContentResponse contentResponse = new FailedContentResponse();
		contentResponse.setFailedcontents(java.util.Collections.emptyList());
		mockServer.expect(ExpectedCount.once(), requestTo(new URI(url))).andExpect(method(HttpMethod.GET))
				.andRespond(withStatus(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON)
						.body(objectMapper.writeValueAsBytes(contentResponse)));
		FailedContentResponse failedContentResponse = subscriptionExtendedClient.getFailedContent(failedContentRequest);
		assertTrue(failedContentResponse.getFailedcontents().isEmpty());
	}

	@Test
	public void getFailedContentInvalidJsonReturnsNullTest() throws Exception {
		mockServer.expect(ExpectedCount.once(), requestTo(new URI(expectedFailedContentURL)))
				.andExpect(method(HttpMethod.GET))
				.andRespond(withStatus(HttpStatus.OK).contentType(MediaType.APPLICATION_JSON).body("not-json"));
		assertNull(subscriptionExtendedClient.getFailedContent(failedContentRequest));
	}
