...
public class MISPLicenseGeneratorExceptionTest {
	...
	public void lengthNotSameExceptionTest() {
		...
		try (MockedStatic<RandomStringUtils> mocked = Mockito.mockStatic(RandomStringUtils.class)) {
			mocked.when(() -> RandomStringUtils.random(Mockito.eq(licenseKeyLength), Mockito.eq(0), Mockito.eq(0),
					Mockito.eq(true), Mockito.eq(true), Mockito.eq(null), Mockito.any(SecureRandom.class)))
