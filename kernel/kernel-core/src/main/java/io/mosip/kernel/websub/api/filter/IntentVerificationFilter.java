...
public class IntentVerificationFilter extends OncePerRequestFilter {
	...
	/**
	 * Callback path to subscribed topic, set by {@link IntentVerificationConfig}.
	 */
	@Setter
	private Map<String, String> mappings = null;

	/**
	 * Creates a filter that uses {@code intentVerifier} for hub GET verification.
	 *
	 * @param intentVerifier matcher for topic (and optionally mode)
	 */
	...
	/**
	 * Handles hub GET intent verification for registered callbacks; otherwise
	 * continues the chain.
	 *
	 * @param request     incoming request
	 * @param response    servlet response (challenge is written on success)
	 * @param filterChain remaining filters / servlet
	 * @throws ServletException if the chain fails
	 * @throws IOException      if the challenge cannot be written
	 */
	@Override
	...
	/**
	 * Resolves the subscribed topic for this request URI, including templated paths
	 * such as {@code /callback/{id}}.
	 *
	 * @param requestURI servlet path of the incoming request
	 * @return mapped topic, or {@code null} if this is not a registered callback
	 */
