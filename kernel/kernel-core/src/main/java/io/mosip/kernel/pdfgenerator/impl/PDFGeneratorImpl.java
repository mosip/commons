...
public class PDFGeneratorImpl implements PDFGenerator {
	...
	/** PDF owner password from {@code mosip.kernel.pdf_owner_password} used with user-password encryption. */
	@Value("${mosip.kernel.pdf_owner_password:\"\"}")
	private String pdfOwnerPassword;

	/** Classpath or filesystem glob of TTF files from {@code mosip.kernel.pdfgenerator.ttf.file.path}. */
	@Value("${mosip.kernel.pdfgenerator.ttf.file.path:classpath:/pdf-generator/*.ttf}")
	private String ttfFilePath;

	/**
	 * Renders HTML from {@code is} to a PDF {@link OutputStream}.
	 * <p>
	 * HTML is normalized to well-formed XHTML via Jsoup, then drawn with
	 * OpenHTMLToPDF using fonts from {@link #ttfFilePath}.
	 * </p>
	 *
	 * @param is HTML input; must be non-null and non-empty
	 * @return PDF bytes in a {@link ByteArrayOutputStream}
	 * @throws IOException            if the stream cannot be read
	 * @throws PDFGeneratorException  if the input is empty or rendering fails
	 */
	@Override
	...
	/**
	 * Normalizes HTML to well-formed UTF-8 XHTML for OpenHTMLToPDF.
	 *
	 * @param is HTML input
	 * @return XHTML string
	 * @throws IOException if {@code is} cannot be read
	 */
	...
	/**
	 * Renders HTML {@code template} to a PDF {@link OutputStream}.
	 *
	 * @param template HTML string; must be non-empty
	 * @return PDF bytes in a {@link ByteArrayOutputStream}
	 * @throws IOException           unused checked signature from {@link PDFGenerator}
	 * @throws PDFGeneratorException if {@code template} is empty or rendering fails
	 */
	@Override
	...
	/**
	 * Writes a plain-text template file as a single-page Helvetica PDF.
	 *
	 * @param templatePath   path of the source text file
	 * @param outputFilePath directory of the output PDF
	 * @param outputFileName file name without {@code .pdf}
	 * @throws IOException           unused checked signature from {@link PDFGenerator}
	 * @throws PDFGeneratorException if PDFBox cannot write the file
	 */
	@Override
	...
	/**
	 * Renders HTML from {@code is}; {@code resourceLoc} is ignored.
	 *
	 * @param is          HTML input
	 * @param resourceLoc unused base URI for relative resources
	 * @return PDF bytes in a {@link ByteArrayOutputStream}
	 * @throws IOException           if the stream cannot be read
	 * @throws PDFGeneratorException if the input is empty or rendering fails
	 */
	...
	/**
	 * Places each image on its own A4 page and returns the PDF bytes.
	 *
	 * @param bufferedImages images to embed
	 * @return PDF bytes
	 * @throws IOException if PDFBox cannot write the document
	 */
	@Override
	...
	/**
	 * Concatenates PDFs loaded from {@code pdfFiles} into one document.
	 *
	 * @param pdfFiles URLs of PDF files to merge
	 * @return merged PDF bytes
	 * @throws IOException if a URL cannot be opened or merge fails
	 */
	@Override
	...
	/**
	 * Optionally encrypts {@code pdf} with {@code password}, then applies a visible PKCS#7 signature.
	 *
	 * @param pdf              PDF bytes to sign
	 * @param rectangle        visible signature box (lower-left and upper-right)
	 * @param reason           signature reason written into the field
	 * @param pageNumber       zero-based page index for the visual signature
	 * @param provider         JCA provider used by {@link SignatureHandler}
	 * @param certificateEntry signing key and X.509 chain
	 * @param password         user password for encryption; blank skips encryption
	 * @return signed (and possibly encrypted) PDF in a {@link ByteArrayOutputStream}
	 * @throws IOException           if PDFBox cannot load or save the document
	 * @throws PDFGeneratorException if incremental save / signing fails
	 */
	@Override
	...
	/**
	 * Returns the certificate subject's CN, or empty if none.
	 *
	 * @param x500CertPrincipal subject principal
	 * @return common name, or {@code ""}
	 */
	...
	/**
	 * Converts a Java2D rectangle to a {@link PDRectangle}.
	 *
	 * @param rectangle2D lower-left origin plus width/height
	 * @return PDFBox rectangle
	 */
	...
	/**
	 * Builds a visual signature appearance PDF with signer name, date, and reason.
	 *
	 * @param signName  CN drawn in the appearance
	 * @param reason    signature reason
	 * @param rectangle appearance bounding box
	 * @return PDF bytes of the appearance template
	 * @throws IOException if the appearance document cannot be written
	 */
	...
	/**
	 * Wraps {@code text} to {@code maxWidth} using {@code font} metrics, honoring explicit newlines.
	 *
	 * @param text     text that may contain {@code \n}
	 * @param font     PDFBox font used for width measurement
	 * @param fontSize font size in points
	 * @param maxWidth maximum line width in user space
	 * @return wrapped lines
	 * @throws IOException if font width cannot be measured
	 */
	...
	/**
	 * Rejects a {@code null} or empty HTML input stream.
	 *
	 * @param dataInputStream HTML input
	 * @throws IOException unused checked signature
	 * @throws PDFGeneratorException if the stream is null or empty
	 */
