...
/**
 * Unchecked exception thrown when authorization (AuthZ) fails with one or more
 * MOSIP service errors.
 * <p>
 * Contract: constructed with a non-null error list from the authorization
 * provider. Callers should inspect {@link #getList()} rather than relying on
...
public class AuthZException extends BaseUncheckedException {
	...
	/**
	 * Service errors returned by the authorization provider; never mutated after
	 * construction.
	 */
	private final List<ServiceError> list;

	/**
	 * Constructs an authorization exception wrapping the given service errors.
	 *
	...
	/**
	 * Returns the authorization errors supplied at construction.
	 *
