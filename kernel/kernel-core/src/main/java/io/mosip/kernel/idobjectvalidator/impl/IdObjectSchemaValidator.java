...
public class IdObjectSchemaValidator implements IdObjectValidator {
	...
	/**
	 * Validates {@code identityObject} against {@code identitySchema}.
	 * <p>
	 * Converts fge processing errors into {@link ServiceError} list entries for
	 * missing and invalid fields. {@code requiredFields} filters which missing
	 * properties are reported (pipe-separated alternatives are OR'd).
	 * </p>
	 *
	 * @param identitySchema JSON schema string
	 * @param identityObject identity object (serialized via Jackson)
	 * @param requiredFields mandatory fields; {@code null} or empty reports all missing properties
	 * @return {@code true} if valid
	 * @throws IdObjectValidationFailedException if schema validation reports errors
	 * @throws IdObjectIOException               if the object cannot be serialized or processed
	 * @throws InvalidIdSchemaException          if the schema cannot be loaded
	 */
	@Override
	...
	/**
	 * Appends a {@link ServiceError} for a missing or unwanted field {@code element}.
	 *
	 * @param identityJson            identity JSON (unused)
	 * @param errorList               list to append
	 * @param processingMessageAsJson fge error JSON
	 * @param errorConstant           MOSIP error constant
	 * @param element                 field name relative to the pointer
	 */
