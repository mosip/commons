...
REM Standalone Windows cmd runner for kernel-config-server (no .ps1).
REM Linux / macOS / Git Bash: use run-local.sh
REM
REM   run-local.bat init | start | smoke | stop | test | all
REM Optional: set MOSIP_CONFIG_DIR=C:\path\to\mosip-config\config
...
set "PID_FILE=%PID_DIR%\config-server.pid"
set "LOG_FILE=%LOG_DIR%\config-server.log"
set "ERR_FILE=%LOG_DIR%\config-server.err.log"
set "MODULE=kernel-config-server"

if not defined CONFIG_SERVER_PORT set "CONFIG_SERVER_PORT=51000"
...
:usage
echo Local kernel-config-server ^(local,native profile, :51000 /config^)
echo.
echo   run-local.bat init     package this module
echo   run-local.bat start    run and wait until Spring Boot is ready
echo   run-local.bat smoke    GET /config/actuator/health
echo   run-local.bat stop
echo   run-local.bat test      Maven tests
echo   run-local.bat all      init + test + start + smoke
echo.
echo Port: %CONFIG_SERVER_PORT% ^(override with set CONFIG_SERVER_PORT=...^)
echo Config: src\main\resources\application-local.properties
echo Optional: set MOSIP_CONFIG_DIR=C:\path\to\mosip-config\config
exit /b 1

:port_in_use
netstat -ano | findstr /R /C:":%CONFIG_SERVER_PORT% .*LISTENING" >nul 2>&1
exit /b %ERRORLEVEL%

:pid_on_port
set "PORT_PID="
for /f "tokens=5" %%P in ('netstat -ano ^| findstr /R /C:":%CONFIG_SERVER_PORT% .*LISTENING"') do set "PORT_PID=%%P"
exit /b 0
...
echo ==^> starting config-server from %BOOT_JAR%
...
set "EXTRA_OPT="
if defined MOSIP_CONFIG_DIR (
  set "CONFIG_URI=%MOSIP_CONFIG_DIR:\=/%"
  set "EXTRA_OPT=-Dspring.cloud.config.server.native.search-locations=file:!CONFIG_URI!,classpath:/"
  echo     extra search-locations=file:!CONFIG_URI!
)
...
set "LAUNCH_PS1=%LOCAL_DIR%\launch-config-server.ps1"
> "%LAUNCH_PS1%" echo $ErrorActionPreference = 'Stop'
if defined EXTRA_OPT (
  >> "%LAUNCH_PS1%" echo $argList = @^('-Dspring.profiles.active=local,native','-Dserver.port=%CONFIG_SERVER_PORT%','!EXTRA_OPT!','-jar','%BOOT_JAR%'^)
) else (
  >> "%LAUNCH_PS1%" echo $argList = @^('-Dspring.profiles.active=local,native','-Dserver.port=%CONFIG_SERVER_PORT%','-jar','%BOOT_JAR%'^)
)
...
echo ==^> waiting for Spring Boot startup on port %CONFIG_SERVER_PORT% ^(up to 90s^) ...
call :wait_ready 90
if errorlevel 1 exit /b 1
...
if exist "%PID_FILE%" (
  ...
  if defined PORT_PID (
    ...
    echo warn: ready but PID for port %CONFIG_SERVER_PORT% not resolved
  )
)
echo.
...
if not errorlevel 1 (
  if exist "%LOG_FILE%" (
    findstr /C:"Started ConfigServerBootApplication" "%LOG_FILE%" >nul 2>&1
    if not errorlevel 1 exit /b 0
  )
)

if exist "%LOG_FILE%" (
  findstr /C:"APPLICATION FAILED TO START" /C:"Port %CONFIG_SERVER_PORT% was already in use" "%LOG_FILE%" >nul 2>&1
  if not errorlevel 1 (
...
:wait_ready_fail
echo error: Spring Boot did not become ready within !WAIT_MAX!s on port %CONFIG_SERVER_PORT%
...
echo ==^> stopping config-server ^(%STOP_PID%^) on port %CONFIG_SERVER_PORT%
...
:smoke
call :ensure_dirs
echo ==^> waiting for config-server ^(up to 60s^)
set /a ELAPSED=0
:smoke_loop
...
call :http_ok "http://127.0.0.1:%CONFIG_SERVER_PORT%/config/actuator/health"
if "%HTTP_CODE%"=="200" (
  set "HEALTH_URL=http://127.0.0.1:%CONFIG_SERVER_PORT%/config/actuator/health"
  goto :smoke_ok
)
call :http_ok "http://127.0.0.1:%CONFIG_SERVER_PORT%/actuator/health"
if "%HTTP_CODE%"=="200" (
  set "HEALTH_URL=http://127.0.0.1:%CONFIG_SERVER_PORT%/actuator/health"
  goto :smoke_ok
)
ping -n 3 127.0.0.1 >nul
set /a ELAPSED+=2
...
call :http_ok "http://127.0.0.1:%CONFIG_SERVER_PORT%/config/application/local"
echo config   http://127.0.0.1:%CONFIG_SERVER_PORT%/config/application/local  HTTP %HTTP_CODE%
if not "%HTTP_CODE%"=="200" (
  echo error: config endpoint not healthy
  exit /b 1
)
echo smoke ok
exit /b 0
