...
* **mosip_commons_db_deployment.sh:** This is the .sh file which is present in /home/madmin/database directory and which will be executed for all database deployment in single command execution.
...
**Step 2** -> Deployment on all databases, run the **"mosip_commons_db_deployment.sh"** script which is avialble in the /database directory.  To access **"mosip_all_db_deployment.sh"** script, follow the below given commands:
