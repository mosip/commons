## MOSIP Common module Database scripts inventory and deployment guidelines on postgresql database. 
...
**Example:** the commons module script folder is /**mosip_base_directory**>>db_scripts>>mosip_kernel where all the database scripts related to kernel are available.
...
* **mosip_commons_db_deployment.sh:** This is the .sh file which is present in /home/madmin/database directory and which will be executed for all commons database deployment in single command execution.
...
1) DB Deployment for all common module databases
...
**Step 2** -> Deployment on all common module databases, run the **"mosip_commons_db_deployment.sh"** script which is avialble in the /database directory.  To access **"mosip_commons_db_deployment.sh"** script, follow the below given commands:

<em> **Enter:-bash-4.2$** cd /home/madmin/database/ </em>

<em> **Enter:-bash-4.2$** bash mosip_commons_db_deployment.sh </em>
