### -- ---------------------------------------------------------------------------------------------------------
### -- Script Name		: KEY MANAGER DB Artifacts deploy
### -- Deploy Module 	: MOSIP KEY MANAGER
### -- Purpose    		: To deploy MOSIP KEY MANAGER Database DB Artifacts.       
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('keymgruser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating key manager database user" | tee -a $LOG 2>&1
    ...
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Key Manager user already exist" | tee -a $LOG 2>&1
fi
