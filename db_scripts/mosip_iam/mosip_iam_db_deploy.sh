### -- ---------------------------------------------------------------------------------------------------------
### -- Script Name		: IAM DB Artifacts deploy
### -- Deploy Module 	: MOSIP IAM
### -- Purpose    		: To deploy MOSIP IAM Database DB Artifacts.       
...
echo `date "+%m/%d/%Y %H:%M:%S"` ": Start sourcing the DB scripts" | tee -a $LOG 2>&1
#echo "date:" `date "+%m/%d/%Y %H:%M:%S"`
echo `date "+%m/%d/%Y %H:%M:%S"` ": Path where all the DB scripts exist:$BASEPATH" | tee -a $LOG 2>&1

#========================================DB Deployment process begins on IAM DB SERVER======================================
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('iamuser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating iam database user" | tee -a $LOG 2>&1
    ...
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Iam database user already exist" | tee -a $LOG 2>&1
fi
