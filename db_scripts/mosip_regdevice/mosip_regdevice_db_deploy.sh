...
### -- Purpose    		: To deploy MOSIP REGISTRATION DEVICE MANAGEMENT Database DB Artifacts.       
...
#========================================DB Deployment process begins on REGISTRATION DEVICE MANAGEMENT DB SERVER======================================
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('regdeviceuser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating Registration Device database user" | tee -a $LOG 2>&1
...
#========================================DB Deployment process completes on REGISTRATION DEVICE DB SERVER======================================
